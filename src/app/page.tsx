...
function App() {
  ...
  const [tipStatus, setTipStatus] = useState<{ [key: string]: 'idle' | 'pending' | 'success' | 'error' | string }>({})
  ...
  const fetchCoins = async () => {
    try {
      ...
      const coinsData = response.data?.exploreList?.edges?.map((edge) => edge.node)
      setCoins(coinsData || [])
    } catch (error) {
        ...
        <div className="container" style={{
          ...
          <h2 style={{
            ...
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM12 5C13.66 5 15 6.34 15 8C15 9.66 13.66 11 12 11C10.34 11 9 9.66 9 8C9 6.34 10.34 5 12 5ZM12 19.2C9.5 19.2 7.29 17.92 6 15.98C6.03 13.99 10 12.9 12 12.9C13.99 12.9 17.97 13.99 18 15.98C16.71 17.92 14.5 19.2 12 19.2Z" fill="#4CAF50"/>
            </svg>
            Account Wallet
          </h2>
          <div style={{
            ...
            <div style={{
              ...
              <span style={{
                padding: '4px 8px',
                borderRadius: '4px',
                backgroundColor: account.status === 'connected' ? '#e8f5e9' : '#ffebee',
            ...
            <div style={{
              ...
              <span style={{ fontWeight: '500', color: isDarkMode ? '#cccccc' : '#666' }}>Wallet Address:</span>
              <div style={{
                ...
                {account.status === 'connected' && signData ? (
                  <>
                    {showWalletAddress
                      ? (account.addresses ? truncateAddress(account.addresses[0]) : 'Not connected')
                      : (account.addresses ? maskAddress(account.addresses[0]) : 'Not connected')}
                    <button
            ...
            <div style={{
              ...
              <span style={{ fontWeight: '500', color: '#666' }}>Chain ID:</span>
              <span style={{
                padding: '4px 8px',
          ...
          <div style={{ marginTop: '24px' }}>
            <div style={{ 
              ...
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM13 17H11V11H13V17ZM13 9H11V7H13V9Z" fill="#666"/>
              </svg>
              Sign Message
            </div>
            <button
            ...
            <div key={index} style={{
              ...
              <div style={{ 
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    <button
                      onClick={() => setShowDescription({ ...showDescription, [coin.id]: !showDescription[coin.id] })}
                      style={{
                        background: 'none',
                        border: 'none',
                        color: '#2196f3',
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    {coin.createdAt ? new Date(coin.createdAt).toLocaleString() : 'Date not available'}
                  </div>
                </div>
              </div>
              ...
              <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                {!showTipForm[coin.id] ? (
                  <button
                    ...
                    style={{ padding: '8px 16px', borderRadius: '4px', backgroundColor: '#4CAF50', color: 'white', border: 'none', cursor: 'pointer' }}
                  >
                    Tip Creator
                  </button>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <input
                        ...
                        onChange={e => setTipAmounts({ ...tipAmounts, [coin.id]: e.target.value })}
                      ...
                      <button
                        ...
                        onClick={async () => {
                          try {
                            if (!tipAmounts[coin.id]) return alert('Please enter a tip amount!')
                            if (!coin.creatorAddress || typeof coin.creatorAddress !== 'string' || !coin.creatorAddress.startsWith('0x')) return alert('Invalid creator address!')
                            setTipStatus({ ...tipStatus, [coin.id]: 'pending' })
                            const tx = await sendTransactionAsync({
                              to: coin.creatorAddress as `0x${string}`,
                              value: parseEther(tipAmounts[coin.id]),
                            })
                            setTipStatus({ ...tipStatus, [coin.id]: 'success' })
                        ...
                        style={{ padding: '8px 16px', borderRadius: '4px', backgroundColor: '#4CAF50', color: 'white', border: 'none', cursor: 'pointer' }}
                      >
                        {tipStatus[coin.id] === 'pending' ? 'Sending...' : 'Send Tip'}
                      </button>
                      <button
                        type="button"
                        onClick={() => setShowTipForm({ ...showTipForm, [coin.id]: false })}
                        style={{ padding: '8px 16px', borderRadius: '4px', backgroundColor: '#f44336', color: 'white', border: 'none', cursor: 'pointer' }}
                      >
                        Cancel
                      </button>
                    </div>
              ...
              {tipStatus[coin.id] === 'success' && (
                <div style={{ color: 'green', marginTop: 4 }}>
                  ...
                  {tipTxHash[coin.id] && (
                    <div>
                      <a href={`https://sepolia.basescan.org/tx/${tipTxHash[coin.id]}`} target="_blank" rel="noopener noreferrer" style={{ color: '#1a73e8', textDecoration: 'underline', wordBreak: 'break-all' }}>
                        View on BaseScan
                      </a>
                    </div>
                  )}
              ...
              {tipStatus[coin.id] === 'error' && (
                <div style={{ color: 'red', marginTop: 4 }}>Failed to send tip.</div>
              )}
            </div>
          ))}
        </div>
